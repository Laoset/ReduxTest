...
const IndumentariaCategoria = () => {
  ...
  useEffect(()=> {
    Axios.get("https://api-to-vercel-olive.vercel.app/api/productos/category/Indumentaria")
        .then(response=>{
        ...
        <div className="grid grid-cols-5 mx-4 gap-8 px-6 my-1">
          {
            products.map(product => {
              return (
                <div key={product.id} className="bg-white flex flex-col justify-center items-center border-gray-100 border-solid border-2 w-full h-60">
                  <div className='h-28 p-1 w-30'>
                    <img className='h-28 w-40 rounded-md' src={product.image} alt="" />
                  </div>
                  <div className='h-28 text-base pt-1'>
                    <h4 className='text-black'>{product.name}</h4>
                    <p className='text-black py-1'><b className='font-normal'>Price: $</b>{product.price}</p>
                    <p className='text-black'><b className='font-normal'>Category:</b> {product.category}</p>
                  </div>
                  <div className='p-1'>
                    <button
...
export default IndumentariaCategoria